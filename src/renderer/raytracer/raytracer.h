			...
			float3 right, float3 up, size_t depth, size_t accumulation_num)
	{
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				float u = (2.f * (float)x) / (float)(width - 1) - 1.f;
				float v = (2.f * (float)y) / (float)(height - 1) - 1.f;
				u *= (float)width / height;
				float3 ray_direction = direction*depth + u * right - v * up;
				ray r(position, ray_direction);

				payload p = trace_ray(r, depth);

				render_target->item(x, y) = RT::from_color(p.color);
			}
		}
